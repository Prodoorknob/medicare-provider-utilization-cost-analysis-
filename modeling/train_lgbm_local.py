"""
train_lgbm_local.py -- LightGBM, trained locally, logs to Databricks MLflow

Key advantages over XGBoost at 100M+ rows:
  - GOSS (Gradient-based One-Side Sampling): keeps all large-gradient
    instances + random sample of small-gradient ones -> faster convergence
  - Leaf-wise (best-first) tree growth: deeper trees per iteration than
    XGBoost's level-wise approach, better for heterogeneous data
  - Native categorical support: splits on categorical values directly
    without one-hot or label encoding overhead

Two training modes:
  --mode batch  (default): Incremental training by Census region. Trains on
                4 regions sequentially via init_model parameter.
  --mode full:  Loads all gold parquets into a single Dataset and trains
                in one shot (needs more RAM).

GPU support: auto-detects CUDA via LightGBM's gpu_hist tree method.
...
# Categorical feature names -- LightGBM will use optimal split finding
# on these instead of treating them as continuous ordinal values.
CAT_FEATURE_NAMES = [
...
ROUNDS_PER_REGION = 125   # ~500-625 total across regions


def _detect_device() -> str:
    """Return 'gpu' if LightGBM GPU build is available, else 'cpu'."""
    try:
        # Try a quick GPU train to see if it works
        _d = lgb.Dataset(np.zeros((2, 1)), label=np.zeros(2))
        lgb.train({"device": "gpu", "verbosity": -1, "num_iterations": 1}, _d)
        return "gpu"
    except Exception:
        return "cpu"
...
LGB_PARAMS = {
    ...
    "metric":           "rmse",
    "learning_rate":    0.05,
    "num_leaves":       63,         # leaf-wise: num_leaves controls complexity
    ...
    "seed":             42,
    "verbose":          -1,
    "boosting_type":    "gbdt",     # can switch to "goss" for faster training
    "data_sample_strategy": "goss", # GOSS within GBDT framework
}
...
def _load_region(
    ...
    Returns (X, y_log1p, year) as numpy arrays.
    """
...
def _make_dataset(X, y, active_features, cat_indices):
    """Create a LightGBM Dataset with categorical feature declarations."""
    ds = lgb.Dataset(
        X, label=y,
        feature_name=active_features,
        categorical_feature=[active_features[i] for i in cat_indices],
        free_raw_data=False,
    )
    return ds
...
def train_batch(gold_dir: str, sample: float, active_features: list[str], split: str):
    ...
    run_name = "lgbm_no_charge_local" if ablation else "lgbm_local"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for region_name, states in CENSUS_REGIONS.items():
            ...
            dtrain = _make_dataset(X_train, y_train, active_features, cat_indices)
            dval   = _make_dataset(X_val,   y_val,   active_features, cat_indices)
        ...
        y_pred = booster.predict(X_val_all)
...
# -- Full mode: load everything into one Dataset ------------------------------
...
def train_full(gold_dir: str, sample: float, active_features: list[str], split: str):
    ...
    dtrain = _make_dataset(X_train, y_train, active_features, cat_indices)
    dval   = _make_dataset(X_test,  y_test,  active_features, cat_indices)
